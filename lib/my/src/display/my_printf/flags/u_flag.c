...
** Last update Sun Nov 20 19:14:46 2016 Arthur Chaloin
*/
#include <unistd.h>
#include <stdlib.h>
#include <stdarg.h>
...
int	my_putnbr_short_unsigned(int nbr, char *base);
...
static void	handle_format(const char *format_flag, t_flag_data *data)
{
  if (my_str_contain(MINOR_FLAGS, format_flag[data->cursor]) == -1)
    ...
    {
      data->func = &my_putnbr_short_unsigned;
...
static int	apply_formats(t_flag_data *data)
{
  ...
  if (data->field_len && !data->minus && !data->zero_padded)
    print_space(data->field_len - data->len_func(data->nb, 10)
  ...
  if (data->zero_padded && data->precision <= 0)
    {
      print_zero(data->field_len - data->len_func(data->nb, 10));
    }
  return (0);
}
...
int		u_flag(va_list *args, const char *format_flag)
{
  ...
  flag_init(&data);
  while (format_flag[data.cursor] != 'u')
    {
  ...
  return ((!data.minus) ? data.return_val : data.return_val + print_space(
	  data.field_len - data.len_func(data.nb, 8)
