...
** Last update Sun Nov 20 19:40:31 2016 Arthur Chaloin
*/
#include <unistd.h>
#include <stdlib.h>
#include <stdarg.h>
...
int	my_putnbr_long(int nbr, char *base);
int	my_putnbr_short(int nbr, char *base);
int	my_intlen(int nb, int base);
int	my_long_intlen(int nb, int base);
int	my_short_intlen(int nb, int base);
...
static void	handle_format(const char *format_flag, t_flag_data *data)
{
  if (my_str_contain(MINOR_FLAGS, format_flag[data->cursor]) == -1)
    ...
    {
      data->func = &my_putnbr_long;
      data->len_func = &my_long_intlen;
      data->type = 1;
    }
  else if (format_flag[data->cursor] == 'h')
    {
      data->func = &my_putnbr_short;
      data->len_func = &my_short_intlen;
      data->type = 2;
    }
...
{
  if (data->nb < 0 && data->nb != -2147483648)
    {
      data->field_len--;
      data->plus = 0;
      data->space = 0;
    }
  data->precision -= (data->precision > 0) ? data->len_func(data->nb, 10) : 0;
  if (data->field_len && !data->minus && !data->zero_padded)
    print_space(data->field_len - data->len_func(data->nb, 10) - data->space
		- data->plus - ((data->precision > 0) ? data->precision : 0));
  if (data->space)
    my_putchar(' ');
  else if (data->plus)
    my_putchar('+');
  ...
  if (data->precision)
    print_zero(data->precision);
  if (data->zero_padded && data->precision <= 0)
    print_zero(data->field_len - data->len_func(data->nb, 10) - data->space -
	       data->plus);
  return (0);
}
...
int		d_flag(va_list *args, const char *format_flag)
{
  ...
  flag_init(&data);
  while (format_flag[data.cursor] != 'd' && format_flag[data.cursor] != 'i')
    {
      handle_format(format_flag, &data);
      data.cursor++;
    }
  if (data.type == 1)
    data.nb = va_arg(*args, long int);
  else
    data.nb = va_arg(*args, int);
  data.return_val = apply_formats(&data);
  data.return_val += (data.func)(data.nb, "0123456789");
  return ((!data.minus) ? data.return_val : data.return_val + print_space(
	  data.field_len - data.len_func(data.nb, 10)
	   - data.space - data.plus - ((data.precision > 0) ?
				       data.precision : 0)));
}
