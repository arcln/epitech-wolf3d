...
** Last update Sat Nov 19 20:33:45 2016 Arthur Chaloin
*/
#include <unistd.h>
#include <stdlib.h>
#include <stdarg.h>
...
static int	apply_formats(t_flag_data *data)
{
  if (data->field_len && !data->minus)
    print_space(data->field_len - ((data->precision > 0 && data->precision <
				    data->len_func(data->str, 10)) ?
				   data->precision :
				   data->len_func(data->str, 10)));
  return (0);
}
...
int		ss_flag(va_list *args, const char *format_flag)
{
  ...
  flag_init(&data);
  while (format_flag[data.cursor] != 'S')
    {
  ...
  if (data.precision > 0)
    data.return_val += my_shownstr(data.str, data.precision);
  else
    data.return_val += (data.func)(data.str);
  return ((!data.minus) ? data.return_val : data.return_val + print_space(
          data.field_len - ((data.precision > 0 && data.precision <
      				    data.len_func(data.str, 10)) ?
      				   data.precision :
      				   data.len_func(data.str, 10))));
}
