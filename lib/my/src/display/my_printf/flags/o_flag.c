...
** Last update Sun Nov 20 19:14:56 2016 Arthur Chaloin
*/
#include <unistd.h>
#include <stdlib.h>
#include <stdarg.h>
...
static int	apply_formats(t_flag_data *data)
{
  ...
  data->precision -= (data->precision > 0) ? data->len_func(data->nb, 8) : 0;
  if (data->field_len && !data->minus && !data->zero_padded)
    print_space(data->field_len - data->len_func(data->nb, 8)
  ...
  if (data->zero_padded && data->precision <= 0)
    {
      print_zero(data->field_len - data->len_func(data->nb, 8)
		 - data->alternated);
    }
  return (0);
}
...
int		o_flag(va_list *args, const char *format_flag)
{
  ...
  flag_init(&data);
  while (format_flag[data.cursor] != 'o')
    {
  ...
  data.return_val += (data.func)(data.nb, "01234567");
